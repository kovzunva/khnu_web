...
class WorkService extends BaseItemService 
{
    ...
    public function GetRecommendations(Request $request = null, bool $paginate = true, $per_page = self::PER_PAGE_BASE){
        $this->sort_options = [
            1 => ['name' => 'За рейтингом орієнтира', 'sql' => 'orientator_rating'],
            2 => ['name' => 'За середнім рейтингом', 'sql' => 'rating'],
            3 => ['name' => 'За назвою', 'sql' => 'name'],
            4 => ['name' => 'За датою додавання', 'sql' => 'id'],
        ];
        $this->sort = $this->GetOrderBy($request);
        $count = $this->GetRecommendationsCount();
        $this->paginator = $paginate? $this->GetPaginator($request,$per_page,$count):null;

        $userId = auth()->user()->id;
        $items = DB::select('SELECT w.*, MAX(r.value) as orientator_rating 
                FROM work_view as w        
                JOIN rating r ON w.id = r.w_id
                JOIN orientator o ON o.user_orientator_id = r.user_id
                LEFT JOIN rating user_rating ON w.id = user_rating.w_id AND user_rating.user_id = ?
                LEFT JOIN review user_review ON w.id = user_review.w_id AND user_review.user_id = ?
                WHERE r.value >= 8 AND o.user_id = ?
                AND user_rating.w_id IS NULL
                AND user_review.w_id IS NULL
                GROUP BY w.id'
                .$this->sort->sql
                .$this->paginator->sql, 
    ...
    public function GetRecommendationsCount(){
        $userId = auth()->user()->id;
        $result = DB::select("SELECT count(*) FROM work_view as w        
                JOIN rating r ON w.id = r.w_id
                JOIN orientator o ON o.user_orientator_id = r.user_id
                LEFT JOIN rating user_rating ON w.id = user_rating.w_id AND user_rating.user_id = ?
                LEFT JOIN review user_review ON w.id = user_review.w_id AND user_review.user_id = ?
                WHERE r.value >= 8 AND o.user_id = ? AND user_rating.w_id IS NULL AND user_review.w_id IS NULL
                GROUP BY w.id", [$userId, $userId, $userId]);
